...
-(void)orderInfoShow1:(OrderProfile *)order index:(NSString *)index leiji:(NSString *)touru{
    ...
    NSString * str = [NSString stringWithFormat:@"%@",order.issueNumber];
    NSString * str1 = [str substringFromIndex:str.length - 2];
    qihao.text = [NSString stringWithFormat:@"%@期",str1];
    
    //当期投入
    NSString *stry = [NSString stringWithFormat:@"%@",order.subscription];
    if ([order.payStatus isEqualToString:@"0"] || [order.payStatus isEqualToString:@"2"]) {
        dangqi.text = @"0元";
    }else{
